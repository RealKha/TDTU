...
int available[NUMBER_OF_RESOURCES]; // Khai báo biến available
...
int main(int argc, char **argv)
{
    ...
    else
    {
        ...
        // Lấy tổng số tài nguyên hệ thống có ban đầu từ tham số dòng lệnh
        for (int i = 0; i < NUMBER_OF_RESOURCES; i++)
            total_resources[i] = atoi(argv[i + 1]);

        // Tính tổng tài nguyên đã cấp phát cho các tiến trình
        int allocated_resources[NUMBER_OF_RESOURCES] = {0};
        ...
        // Tính available = tổng tài nguyên ban đầu - tổng đã cấp phát
        for (int i = 0; i < NUMBER_OF_RESOURCES; i++)
        {
            available[i] = total_resources[i] - allocated_resources[i];
        }
    }
...
int safety_algorithm(int *available)
{
    ...
    int work[NUMBER_OF_RESOURCES]; // Sửa kích thước mảng work
    // STEP 1
